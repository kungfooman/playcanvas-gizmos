...
export class RTH_RuntimeGrid {
     ...
     */
    setOptions(options) {
        const {mainCamera, range } = this.toolOptions;
        mainCamera.layers = mainCamera.layers.concat(RTH_RuntimeGrid.layer.id); // 相机添加layer
    ...
    createGrid(w, h, wd = w, hd = h, color = pc.Color.BLACK) {
        ...
        const grid = new pc.Entity('RTH_RuntimeGrid');
