...
export class PathCurve extends Tool<PathCurveOptions, any> {
    ...
    /**
     * 创建路径曲线
     * @param {PathCurveOptions} options 路径曲线选项 
     */
    constructor(options) {
        super();
    ...
    /**
     * 绘制曲线，并随节点更新
     * @param {{ color?: pc.Color, drawNormal?: boolean, drawTangent: boolean }} option 选项
     */
    draw(option) {
    ...
    /**
     * 获得某处坐标
     * @param {number} percent 此位置在曲线上的百分比
     * @returns {pc.Vec3} 此处的坐标
     */
    getPosition(percent) {
     ...
     * @param {number} percent 此位置在曲线上的百分比
     * @param {boolean} [nodeRotation] 是否使用节点的旋转的插值作为曲线的旋转
     * @param {boolean} [inverse] 是否反向
     * @returns {pc.Quat} 此处的旋转
     */
    getRotation(percent, nodeRotation, inverse) {
        if (nodeRotation) {
            // 取朝向插值
     ...
     * 获得某处坐标
     * @param {number} percent 此位置在曲线上的百分比
     * @returns {pc.Vec3} 此处的切线方向
     */
    getTangent(percent) {
        return this.forward.set(
            cast<Curve_EX>(this.px).getTangent(percent),
            cast<Curve_EX>(this.py).getTangent(percent),
            cast<Curve_EX>(this.pz).getTangent(percent)
        ).normalize();
    }
    /**
     * 获得当前所处的节点
     * @param {number} percent 此位置在曲线上的百分比
     * @returns {number} 当前所处节点
     */
    getCurrentPath(percent) {
